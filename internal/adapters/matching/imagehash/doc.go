...
//
// Matcher скачивает контент, извлекает первый кадр, отбрасывает
// low-information изображения и считает goimagehash PerceptionHash. pHash
// используется как MVP image fingerprint, потому что он достаточно устойчив к
// resize, recompression и легким изменениям цвета.
//
...
//
// Low-information guard специально пропускает однотонные и почти однотонные
// изображения. Для них global pHash часто слишком общий и может приводить к
// массовым false positives.
//
// Этот matcher не является semantic или object detector. Сильный crop,
// overlays, collage edits, meme edits и тяжелые redraws все еще могут давать
// false negatives или false positives. Такие случаи нужно закрывать следующими
// слоями модерации: region hashes, PDQ, local features или embeddings после
// калибровки на реальных данных.
package imagehash
