...
Вне scope текущей версии:

- обычные Telegram `Video`;
- Telegram `Document`;
- устойчивость к сильному crop, overlay, watermark, speed change и монтажу;
- semantic matching через embeddings.

Для обычных `Video` и `Document` бот не создает проверок.
