...
def test_faiss_hnsw_index_saves_and_loads_from_disk(tmp_path: Path):
    ...
    loaded = FaissHNSWVectorIndex.load(
        path=index_path,
        dimension=3,
        refs=[IndexedVectorRef(ban_id=ban.id, frame_index=ban.frames[0].frame_index)],
...
def _insert_ban(store: SQLiteVectorStore, *, file_unique_id: str, vectors: list[VectorFrame]):
