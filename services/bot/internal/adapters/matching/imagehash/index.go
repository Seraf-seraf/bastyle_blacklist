...
// размер списка ожидается в пределах десятков тысяч записей. PostgreSQL хранит
// данные между перезапусками, а этот индекс загружается из PostgreSQL при старте
// и используется только для быстрого поиска.
...
//
// Метод используется при старте, когда PostgreSQL store возвращает persisted
