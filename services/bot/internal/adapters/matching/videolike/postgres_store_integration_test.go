...
func TestPostgresStoreInsertConflictDoesNotLeaveOrphanMediaBan(t *testing.T) {
	...
	pool := newVideoLikePostgresPool(t, ctx)
	...
	hash := StoredVideoLikeHash{
		...
		Frames: []StoredVideoLikeFrameHash{
			{FrameIndex: 0, PositionMillis: 0, Hash: 1},
			{FrameIndex: 1, PositionMillis: 1000, Hash: 2},
		},
	}

	if _, err := store.insert(ctx, hash); err != nil {
		t.Fatalf("первичная вставка videolike: %v", err)
	}
	...
	if err := insertMediaBan(ctx, tx, duplicateBanUID, hash.ChatID, hash.SourceType, "duplicate-file-unique-id"); err != nil {
		t.Fatalf("вставка duplicate media_ban: %v", err)
	}
	if _, err := store.Insert(ctx, tx, duplicateBanUID, hash); err != nil {
		t.Fatalf("конфликтная вставка videolike: %v", err)
	}
...
SELECT count(*)
FROM media_ban mb
LEFT JOIN blocked_video_like bvl ON bvl.ban_uid = mb.ban_uid
WHERE bvl.ban_uid IS NULL
...
func newVideoLikePostgresPool(t *testing.T, ctx context.Context) *pgxpool.Pool {
	t.Helper()
